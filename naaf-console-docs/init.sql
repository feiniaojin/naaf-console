...
CREATE TABLE `sys_account` (
  ...
  `account_id` varchar(64) NULL COMMENT '账号,业务使用',
...
CREATE TABLE `sys_account_rel_user` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '自增主键,业务不用',
  `account_id` varchar(64) NULL COMMENT '账号id,业务使用',
  ...
  `version` bigint DEFAULT 1 COMMENT '乐观锁',
  PRIMARY KEY (`id`),
    index `idx_uid_accountId`(`uid`,`account_id`),
  index `idx_accountId_uid`(`account_id``,`uid`)
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_bin COMMENT = '账号与用户关联表';
